...
def route_task(task: str, repo: pathlib.Path | str | None = None, forced_type: str | None = None) -> dict[str, Any]:
    ...
    if forced_type is not None:
        if forced_type not in ROUTES:
            raise ValueError(f"Unknown forced task type: {forced_type}")
    ...
    complexity = classify_complexity(task, selected)
    risk = classify_risk(task, selected)
    optional: list[str] = ["harness.complexity", "harness.risk-routing", "harness.model-routing", "orchestration.scheduler", "context.lane-budget", "quality.gate", "harness.retry-policy"]
    if complexity["multi_agent_recommended"]:
        optional += ["context.handoff", "context.artifact", "orchestration.handoff"]
    ...
    result: dict[str, Any] = {
        "task_type": selected,
        "classification": "explicit" if forced_type else "heuristic",
        "scores": scores,
