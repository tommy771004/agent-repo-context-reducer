...
def search_repository(root: pathlib.Path, query: str, max_results: int = 50) -> dict[str, Any]:
    """Resolve repository.search. Uses rg when available; native index fallback otherwise belongs to caller."""
    resolution = resolve_capability(root, "repository.search")
    selected = resolution["selected"]
    if selected.get("id") != "cli:rg":
        return {"used": False, "resolution": resolution, "results": []}
    started = time.perf_counter()
    proc = subprocess.run(
        [selected["source"], "-n", "--no-heading", "--color", "never", "-m", "3", query, str(root)],
    ...
    for line in proc.stdout.splitlines()[:max_results]:
        # path:line:text; split only the first two separators so Windows drive letters are tolerated poorly only when root is absolute.
        parts = line.split(":", 2)
        if len(parts) == 3:
            p, ln, text = parts
            try:
                rel = pathlib.Path(p).resolve().relative_to(root.resolve()).as_posix()
            except Exception:
                rel = p.replace("\\", "/")
