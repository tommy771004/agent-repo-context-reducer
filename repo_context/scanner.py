...
def detect_framework_hints(manifest_texts: dict[str, str]) -> list[str]:
    ...
    patterns = {
        ...
        "NestJS": ["@nestjs/"], "Tauri": ["@tauri-apps", "tauri ="],
        ...
        "Spring": ["spring-boot", "org.springframework"], "Rails": ["rails"],
...
def _is_entry_point(rel: str, name: str) -> bool:
    ...
    if rel in {"main.py", "app.py", "manage.py", "__main__.py", "index.js", "index.ts", "index.tsx", "main.js", "main.ts", "main.tsx", "server.js", "server.ts", "Program.cs", "main.rs", "lib.rs", "main.go", "App.tsx", "App.jsx"}:
        return True
    if len(parts) == 2 and parts[0] in {"src", "app"} and name in ENTRY_NAMES:
        return True
...
def _static_importance(path: str, summary: dict[str, Any], manifest: bool, entry: bool) -> int:
    ...
    score += min(len(summary.get("imports", [])), 8)
    score += min(len(summary.get("classes", [])) * 2, 8)
    score += min(len(summary.get("routes", [])) * 2, 8)
    return score
...
def _candidate_files(root: pathlib.Path, include_hidden: bool) -> tuple[list[pathlib.Path], str]:
    ...
    return _walk_files(root, include_hidden), "filesystem-walk"
...
def build_index(root: pathlib.Path | str, max_files: int = 10000, max_file_bytes: int = 512_000,
                include_hidden: bool = False, use_cache: bool = True,
                include_generated: bool = False) -> dict[str, Any]:
    root = pathlib.Path(root).resolve()
    if not root.exists() or not root.is_dir():
        raise ValueError(f"Not a directory: {root}")

    candidate_paths, listing_mode = _candidate_files(root, include_hidden)
    ...
    for path in candidate_paths:
        ...
        try:
            ...
            continue
        if rel.startswith(".repo-context/") or rel.startswith(".repo-context-cache/"):
            stats["internal_state_skipped"] += 1
            continue
        if is_secret_path(rel):
            stats["secret_paths_skipped"] += 1
            continue
        if not include_generated and is_generated_path(rel):
            stats["generated_paths_skipped"] += 1
            continue
        ...
        entry = _is_entry_point(rel, path.name)
        summary = None
        key = ""
        if source and use_cache:
            try:
                key = file_cache_key(path)
                summary = cache.get(rel, key)
            except OSError:
                key = ""
        ...
        if summary is None or manifest or not source:
            ...
            if text is None:
                stats[f"{reason or 'unreadable'}_skipped"] += 1
                continue
            if not include_generated and looks_generated(text):
                stats["generated_content_skipped"] += 1
                continue

        if manifest:
            manifests.append(rel)
            manifest_texts[rel] = (text or "")[:100_000]
        if entry:
            entries.append(rel)
        if source:
            language_counts[SOURCE_EXTENSIONS[ext]] += 1
            stats["source_files"] += 1
        directory_counts[path.parent.relative_to(root).as_posix() or "."] += 1

        if summary is None:
            summary = summarize_source(rel, text or "") if source else {
                "imports": [], "classes": [], "types": [], "functions": [], "exports": [], "routes": [],
                "symbols": [], "symbol_details": [], "lines": len((text or "").splitlines()), "parser": "manifest/config",
            }
            if source:
                summary["symbol_details"] = extract_symbol_index(rel, text or "")
                stats["cache_misses"] += 1
            if source and use_cache and key:
                cache.put(rel, key, summary)
        else:
            stats["cache_hits"] += 1

        try:
            stat_fingerprint = file_cache_key(path)
        except OSError:
            stat_fingerprint = ""
        item = {
            "path": rel, "language": SOURCE_EXTENSIONS.get(ext), "bytes": size,
            "stat_fingerprint": stat_fingerprint,
            "lines": summary["lines"], "imports": summary.get("imports", []),
            "classes": summary.get("classes", []), "types": summary.get("types", []),
            "functions": summary.get("functions", []), "exports": summary.get("exports", []),
            "routes": summary.get("routes", []), "symbols": summary.get("symbols", []),
            "symbol_details": summary.get("symbol_details", []),
            "parser": summary.get("parser"), "manifest": manifest, "entry_point": entry,
        }
        item["static_importance"] = _static_importance(rel, item, manifest, entry)
        files.append(item)

    cache.save()
    graph = build_dependency_graph(files)
    ranked = rank_files(files, graph, sorted(set(entries)))
    by_path = {f["path"]: f for f in ranked}
    stats["files_scanned"] = len(files)
    stats["source_bytes_considered"] = raw_bytes
    return {
        "root": str(root), "listing_mode": listing_mode, "files": ranked, "by_path": by_path,
        "graph": graph, "entry_points": sorted(set(entries)), "manifests": sorted(set(manifests)),
        "languages": dict(language_counts.most_common()),
        "framework_hints": detect_framework_hints(manifest_texts),
        "directories": [{"path": p, "files": c} for p, c in directory_counts.most_common(60)],
        "workspaces": detect_workspaces(root), "stats": dict(stats),
    }


def compact_file(f: dict[str, Any]) -> dict[str, Any]:
    return {
        "path": f["path"], "language": f.get("language"), "lines": f.get("lines"),
        "imports": f.get("imports", []), "classes": f.get("classes", []),
        "types": f.get("types", []), "functions": f.get("functions", []),
        "exports": f.get("exports", []), "routes": f.get("routes", []),
        "rank_score": f.get("rank_score"), "rank_reasons": f.get("rank_reasons", []),
    }
...
def project_map(index: dict[str, Any], top_k: int = 25, query: str | None = None) -> dict[str, Any]:
    ...
    result = {
        "project": {
            ...
            "listing_mode": index["listing_mode"],
            "files_scanned": index["stats"].get("files_scanned", 0),
            "languages": index["languages"], "framework_hints": index["framework_hints"],
            "manifests": index["manifests"], "workspaces": index["workspaces"],
        },
        "query": query,
        "entry_points": index["entry_points"],
        "directories": index["directories"][:30],
        "important_files": selected,
        "central_files": [{"path": p, **d} for p, d in central],
        "stats": {
            **{k: v for k, v in index["stats"].items() if k != "source_bytes_considered"},
            "source_bytes_considered": index["stats"].get("source_bytes_considered", 0),
            "estimated_raw_tokens": estimate_tokens_from_bytes(index["stats"].get("source_bytes_considered", 0)),
            "top_k": top_k,
            "output_note": "Default output includes only ranked Top-K structural summaries, not every file.",
        },
    }
    ...
    raw = index["stats"].get("source_bytes_considered", 0)
    result["stats"]["estimated_reduction_ratio"] = round(max(0.0, 1 - out_bytes / raw), 4) if raw else 0.0
...
def dependency_view(index: dict[str, Any], path: str, depth: int = 1) -> dict[str, Any]:
    ...
    graph = index["graph"]
    return {
        "path": rel, "imports": graph.get("edges", {}).get(rel, []),
        "imported_by": graph.get("reverse", {}).get(rel, []),
        "neighborhood": neighborhood(graph, [rel], depth=depth),
        "unresolved_relative_imports": graph.get("unresolved", {}).get(rel, []),
    }
