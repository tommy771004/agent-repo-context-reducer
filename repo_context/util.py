...
MANIFEST_NAMES = {
    ...
    "Gemfile", "composer.json", "Package.swift", "Dockerfile",
    "docker-compose.yml", "docker-compose.yaml", "pnpm-workspace.yaml",
...
DEFAULT_IGNORE_DIRS = {
    ...
    ".repo-context-cache", ".repo-context",
}

SECRET_NAME_PATTERNS = [
    re.compile(r"(^|/)\.env(?:\.|$)", re.I),
    re.compile(r"(^|/)(?:secrets?|credentials?)(?:\.|/|$)", re.I),
    re.compile(r"\.(?:pem|key|p12|pfx|jks)$", re.I),
    re.compile(r"(?:id_rsa|id_ed25519)$", re.I),
]
GENERATED_PATH_PATTERNS = [
    re.compile(r"(?:^|/)(?:generated|gen|dist|build|coverage|vendor)(?:/|$)", re.I),
    re.compile(r"(?:\.min\.(?:js|css)|\.g\.dart|\.designer\.cs)$", re.I),
]
...
def is_secret_path(rel: str) -> bool:
    return any(p.search(rel) for p in SECRET_NAME_PATTERNS)


def is_generated_path(rel: str) -> bool:
    return any(p.search(rel) for p in GENERATED_PATH_PATTERNS)


def looks_generated(text: str) -> bool:
    head = "\n".join(text.splitlines()[:8]).lower()
    markers = ("code generated", "auto-generated", "autogenerated", "do not edit", "generated by")
...
def safe_read_text(path: pathlib.Path, max_bytes: int) -> tuple[str | None, int, str | None]:
    try:
        if path.is_symlink():
            return None, 0, "symlink"
        size = path.stat().st_size
        if size > max_bytes:
            return None, size, "large"
...
def file_cache_key(path: pathlib.Path) -> str:
    st = path.stat()
    raw = f"{path.resolve()}|{st.st_mtime_ns}|{st.st_size}".encode("utf-8")
