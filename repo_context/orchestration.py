...
def plan_harness(task: str, repo: pathlib.Path | str='.', forced_type: str | None=None, *, context_tokens:int=12000, output_tokens:int=4000, model_calls:int=10, route_result:dict[str,Any]|None=None)->dict[str,Any]:
    ...
    optional=['harness.complexity','harness.risk-routing','harness.model-routing','orchestration.scheduler','context.lane-budget','quality.gate','harness.retry-policy','runtime.adapter','runtime.telemetry','orchestration.parallel']
    if complexity['multi_agent_recommended']:
        optional += ['context.handoff','context.artifact','context.fan-in','context.contradiction','context.synthesis-packet','context.schema','context.trust-boundary','context.streaming','context.tokenizer','context.candidate-detection','context.deterministic-verifier','context.git-provenance','context.model-packet','context.model-context','context.control-plane','context.adaptive-reduction','quality.token-economics','quality.scenario-simulation','orchestration.handoff','quality.grader','quality.reducer-benchmark','quality.final-answer-evaluation','runtime.execute','runtime.cancellation','runtime.backpressure']
