...
def _statement_end(lines: list[str], start: int, limit: int = 200) -> int:
    """End of a `;`-terminated statement, used for languages without brace blocks."""
    for i in range(start - 1, min(len(lines), start - 1 + limit)):
        if ";" in lines[i]:
            return i + 1
...
# Applied only to their own extensions so existing languages keep their current behavior.
EXT_PATTERNS: dict[str, list[tuple[str, Any]]] = {
    "c": [("function", re.compile(r"^\s*(?:[A-Za-z_][\w:<>,\s\*&]*?[\s\*&])([A-Za-z_]\w*)\s*\([^;]*\)\s*(?:const\s*)?\{"))],
    "shell": [
        ("function", re.compile(r"^\s*(?:function\s+)?([A-Za-z_][\w-]*)\s*\(\s*\)\s*\{")),
        ("function", re.compile(r"^\s*function\s+([A-Za-z_][\w-]*)\s*\{")),
    ],
    "ps": [("function", re.compile(r"^\s*function\s+([A-Za-z_][\w-]*)", re.I))],
    "sql": [(
        "type",
        re.compile(
            r"^\s*CREATE\s+(?:OR\s+REPLACE\s+)?(?:TEMP(?:ORARY)\s+|GLOBAL\s+|UNIQUE\s+|MATERIALIZED\s+)?"
...
def _pattern_group(ext: str) -> str | None:
...
def _heuristic_symbols(path: str, text: str) -> list[dict[str, Any]]:
    ...
    out: list[dict[str, Any]] = []
    group = _pattern_group(pathlib.Path(path).suffix.lower())
    patterns = SHARED_PATTERNS + EXT_PATTERNS.get(group or "", [])
    for no, line in enumerate(lines, 1):
        for kind, pat in patterns:
            m = pat.search(line)
            if m:
                if group == "c" and m.group(1) in C_NON_FUNCTIONS:
                    continue
                end = _statement_end(lines, no) if group == "sql" else _brace_end(lines, no)
