...
def state_inventory(repo: pathlib.Path | str) -> dict[str, Any]:
    ...
    return {
        "root": str(root), "state_dir": str(folder), "exists": folder.exists(),
        "regenerable": regenerable, "preserved": preserved, "unrecognized": unknown,
...
def remove_state(repo: pathlib.Path | str, yes: bool = False, include_preserved: bool = False) -> dict[str, Any]:
    root = pathlib.Path(repo).resolve(); inventory = state_inventory(root)
    targets = list(inventory["regenerable"])
    if include_preserved: targets += inventory["preserved"] + inventory["unrecognized"]
    removed=[]; failures=[]
    if yes:
        for item in targets:
            error=_delete(pathlib.Path(item["path"]))
            if error: failures.append({"path":item["path"],"error":error})
            else: removed.append(item["path"])
        folder=state_dir(root)
    ...
    return {"target":"state","root":str(root),"dry_run":not yes,
            ...
            "note": ("Dry run: nothing was removed. Re-run with --yes to apply." if not yes else "Removal applied.") +
                    (" Provider trust, provider manifests and artifacts were kept; pass --all to remove them too." if kept else "")}


def remove_shortcuts(repo: pathlib.Path | str, hosts: tuple[str, ...] = HOSTS,
                     scopes: tuple[str, ...] = ("project",), yes: bool = False,
                     force: bool = False) -> dict[str, Any]:
    results=[uninstall_host_commands(repo,host,scope=scope,yes=yes,force=force) for host in hosts for scope in scopes]
    return {"target":"shortcuts","dry_run":not yes,"hosts":list(hosts),"scopes":list(scopes),"results":results,
...
def remove_artifacts(repo: pathlib.Path | str, yes: bool = False) -> dict[str, Any]:
    root=pathlib.Path(repo).resolve(); store=ArtifactStore(root); items=store.list(limit=10_000); removed=[]
    if yes:
        for item in items:
            error=_delete(store.dir / f"{item['id']}.json")
            if not error: removed.append(item["id"])
    return {"target":"artifacts","root":str(root),"dry_run":not yes,
            "planned":[{"id":i["id"],"producer":i.get("producer"),"bytes":i.get("bytes")} for i in items],
            "removed":removed,"note":"Dry run: nothing was removed. Re-run with --yes to apply." if not yes else "Removal applied."}


def self_update_hint(repo: pathlib.Path | str) -> dict[str, Any]:
    cli=shutil.which("repo-context")
    skill_installed=any((pathlib.Path(p).expanduser()/"agent-repo-context-reducer").exists() for p in
        ("~/.claude/skills","~/.agents/skills","~/.cursor/skills","~/.codex/skills",".claude/skills",".agents/skills"))
    return {"target":"self","executed":False,
            "reason":"This runtime never invokes package managers; it reports the command for you to run.",
            "cli_on_path":cli,"skill_package_detected":skill_installed,
            "commands":{"skill_package":"npx skills update agent-repo-context-reducer",
...
def update_index(repo: pathlib.Path | str, **index_kwargs: Any) -> dict[str, Any]:
    root=pathlib.Path(repo).resolve(); before=index_status(root); result=ensure_index(root,sync=True,**index_kwargs)
    return {"target":"index","mode":result["mode"],"path":result["path"],"previously_indexed":before.get("indexed",False),
            "files":len(result["index"].get("files",[])),"sync_stats":result["index"].get("sync_stats",{})}
    ...
    for host in hosts:
        for scope in scopes:
            ...
            if not any(item["installed"] for item in status["commands"]):
                results.append({"host":host,"scope":scope,"action":"skipped","reason":"no shortcuts installed for this host/scope"}); continue
            written=install_host_commands(repo,host,scope=scope,dry_run=dry_run)
            results.append({"host":host,"scope":scope,"action":"re-rendered","runtime":written["runtime"],"written":written["written"]})
    return {"target":"shortcuts","dry_run":dry_run,"results":results,
